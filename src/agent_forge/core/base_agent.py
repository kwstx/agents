...
class BaseAgent(ABC):
    def __init__(self, agent_id: str, message_bus: MessageBus, log_dir: str = "logs"):
        ...
        self.enable_checkpoints = True # Default enabled for backward compat
    ...
    async def setup_memory(self):
        ...
        from agent_forge.utils.memory import Memory
        import yaml
    ...
    def save_checkpoint(self, task: Any, result: Any = None):
        ...
        # Check config if checkpoints are enabled
        # Ideally this comes from a global config or agent init
        # For now, we check if log_dir was set to None/False in init, but base init doesn't take log_dir storage config explicitly
        # Let's inspect self.memory_module or similar?
        # Or just add a flag self.enable_checkpoints default True
        if not getattr(self, "enable_checkpoints", True):
            return
